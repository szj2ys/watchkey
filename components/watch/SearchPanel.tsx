  ...
  return (
    <>
      <button onClick={() => setShowSearch(!showSearch)}
        aria-label="Toggle search" className="p-3 hover:bg-[#272727] rounded-full transition-colors flex-shrink-0">
        ...
        <div className="border-t border-[#272727] px-4 py-3">
          <form onSubmit={handleSearch} className="flex gap-2 max-w-2xl">
            <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
              ...
              className="flex-1 h-10 px-4 bg-[#121212] border border-[#303030] rounded-full text-white placeholder-gray-500 focus:border-blue-500 focus:outline-none text-base md:text-sm" />
            <button aria-label="Submit search" type="submit" disabled={searchLoading || !searchQuery.trim()}
      ...
      {searchResults.length > 0 && (
        <div className="p-4 lg:p-6 border-b border-[#272727]">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => setSearchResults([])} aria-label="Clear search results" className="text-xs p-3 text-gray-400 hover:text-white">Clear</button>
          </div>
              ...
              <button key={v.id} onClick={() => onAnalyze(v.id)} className="group text-left">
                <div className="relative aspect-video rounded-xl overflow-hidden bg-[#1a1a1a] mb-2">
                  {v.thumbnail ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center"><Play className="w-8 h-8 text-gray-400" /></div>
                  )}
